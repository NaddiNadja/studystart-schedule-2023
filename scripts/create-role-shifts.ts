import { Shift, roleShiftsLine } from "./types";

export const createShifts = (csv_row: roleShiftsLine) => {
    var shifts: Shift[] = [];
    var playbook = csv_row.playbook;

    // 1
    if (csv_row.start1 && csv_row.start1.length) {
    ...
    // 2
    if (csv_row.start2 && csv_row.start2.length) {
    ...
    // 3
    if (csv_row.start3 && csv_row.start3.length) {
    ...
    // 4
    if (csv_row.start4 && csv_row.start4.length) {
    ...
    // 5
    if (csv_row.start5 && csv_row.start5.length) {
    ...
    // 6
    if (csv_row.start6 && csv_row.start6.length) {
    ...
    // 7
    if (csv_row.start7 && csv_row.start7.length) {
